    ...
    <div className={cn('w-full', className)}>
      ...
      <Listbox value={value} onChange={onChange} disabled={disabled} multiple>
        <div className="relative">
          <Listbox.Button
            className={cn(
              'relative w-full cursor-default rounded-lg border border-gray-300 bg-white py-2 pr-10 pl-3 text-right focus:border-[#D4AF37] focus:outline-none focus:ring-2 focus:ring-[#D4AF37] disabled:bg-gray-100 disabled:cursor-not-allowed dark:bg-gray-800 dark:border-gray-700',
      ...
      {selectedOptions.length > 0 && (
        <div className="mt-2 flex flex-wrap gap-2">
          {selectedOptions.map((option) => (
            <span
              key={option.value}
              className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-sm bg-[#D4AF37] bg-opacity-20 text-[#D4AF37]"
            >
              {option.label}
              <button
                type="button"
                onClick={() => removeOption(option.value)}
                className="hover:bg-[#D4AF37] hover:bg-opacity-30 rounded-full p-0.5"
              >
